...
public class MemberUse {
	public static void main(String[] args) {
		FileOutputStream fos = null; //파일저장하기 위한 출력스트림
		...
		try {
			...
			//직렬화(Serialization) : 메모리에 있는 것을 끄집어 내와서 프로그램으로 왔다가 파일로 가져가는 로직
			// 메모리=> 프로그램 => 파일
			oos = new ObjectOutputStream(fos);
			oos.writeObject(m1); //m1 객체를 파일에 저장
			oos.writeObject(m2); //m2 객체를 파일에 저장
			oos.writeObject(m3); //m3 객체를 파일에 저장
			System.out.println("객체를 파일로 저장했습니다.");
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		//역직렬화 처리(de-Serialization) : 파일을 프로그램으로 가져와서 메모리에 넣는것
		// 파일 => 프로그램 => 메모리
