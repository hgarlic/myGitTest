...
public class MuliChatClient {
	...
	public static void main(String[] args) {
		...
		try {
			...
			socket = new Socket(serverIp, 7777); //소켓을 생성하여 연결 요청
			System.out.println("서버에 연결되었습니다.");
			Thread receiveThread = new ClientReceiver(socket);
			receiveThread.start(); // run메소드 호출
	...
	static class ClientScreen extends JFrame implements ActionListener{
		...
		public ClientScreen(String nick) {
			...
			//setComponentOrientation 생략해도 되는데, 패널의 배치방향을 설정하는 메소드(좌에서 우로)
			...
			panel.add(jspane1, BorderLayout.NORTH);
			jspane1.setViewportView(ta);
